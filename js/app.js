  ...
  div.innerHTML = `
    <div class="d-flex justify-content-between">
      <div>
        ...
        <div class="small">Postój: ${stopText}</div>
        <div class="small text-muted">Powód: ${item.delayReason || '-'}; Rozkazy: ${item.writtenOrders || '-'}</div>
      </div>
      <div>
        <button class="btn btn-sm btn-outline-danger" onclick="currentReport.sectionE.splice(${idx},1);saveAndRender()">Usuń</button>
      </div>
    </div>`;
  return div;
}

formStation.addEventListener('
