...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const newReportBtn = qs('newReportBtn');
  const takeReportBtn = qs('takeReportBtn');
  const importBtnStart = qs('importBtnStart');
  const importFileStart = qs('importFileStart');

  const reportNumberEl = qs('reportNumber');
  const currentUserEl = qs('currentUser');

  const exportJsonBtn = qs('exportJson');
  const importBtn = qs('importBtn');
  const importFile = qs('importFile');
  const previewPdfBtn = qs('previewPdf');
  const closeReportBtn = qs('closeReport');

  const catEl = qs('cat');
  const tractionEl = qs('traction');
  const trainNumberEl = qs('trainNumber');
  const routeEl = qs('route');
  const trainDateEl = qs('trainDate');

  const tractionList = qs('tractionList');
  const conductorList = qs('conductorList');
  const ordersList = qs('ordersList');
  const stationsList = qs('stationsList');
  const controlsList = qs('controlsList');
  ...
  window.importReportFromJson = async (text) => {
    try {
  ...
  on(newReportBtn, 'click', async () => {
    ...
    await window.createNewReport({ name, id });
  });

  on(takeReportBtn, 'click', async () => {
    const name = qs('userName')?.value?.trim();
    const id = qs('userId')?.value?.trim();
    if (!name || !id) return alert('Podaj imię i nazwisko oraz numer służbowy.');
    await window.takeReportByNumber({ name, id });
  });

  on(importBtnStart, 'click', () => importFileStart && importFileStart.click());
  on(importFileStart, 'change', async (e) => {
  ...
  function renderTractionRow(item, idx) {
    ...
    div.innerHTML = `
      <div><strong>${safeText(item.name)}</strong> (${safeText(item.id)}) · ZDP: ${safeText(item.zdp)} · Lok: ${safeText(item.loco)} [${safeText(item.from)} → ${safeText(item.to)}]</div>
      <div>
  ...
  function renderConductorRow(item, idx) {
    ...
    div.className = 'd-flex justify-content-between align-items-center station-row';
    div.innerHTML = `
      <div><strong>${safeText(item.name)}</strong> (${safeText(item.id)}) · ZDP: ${safeText(item.zdp)} · Funkcja: ${safeText(item.role)} [${safeText(item.from)} → ${safeText(item.to)}]</div>
      <div>
  ...
  function renderOrderRow(item, idx) {
    ...
    div.className = 'd-flex justify-content-between align-items-center station-row';
    div.innerHTML = `
      <div>${safeText(item.text)} <div class="small text-muted">${meta} · Źródło: ${safeText(item.source)}</div></div>
      <div>
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between">
        <div>
          ...
          <div class="small text-muted">Data przyj.: ${safeText(item.dateArr)} · Plan: ${safeText(item.planArr)} · Real: ${safeText(item.realArr)}</div>
          <div class="small">Odchylenie przyj.: <span class="${arrClass}">${arrText}</span></div>
          <div class="small text-muted">Data odj.: ${safeText(item.dateDep)} · Plan: ${safeText(item.planDep)} · Real: ${safeText(item.realDep)}</div>
  ...
  function renderControlRow(item, idx) {
    ...
    div.innerHTML = `
      <div>
        <strong>${safeText(item.by)}</strong> (${safeText(item.id)})
        <div class="small text-muted">${safeText(item.desc)}</div>
        <div class="small text-muted">Uwagi: ${safeText(item.notes)}</div>
      </div>
      <div>
  ...
  function openEditModal(type, idx) {
    ...
    if (type === 'traction') {
      ...
      qs('t_name').value = item.name || '';
      qs('t_id').value = item.id || '';
      qs('t_zdp').value = item.zdp || 'WAW';
      qs('t_loco').value = item.loco || '';
      qs('t_from').value = item.from || '';
      qs('t_to').value = item.to || '';
      ...
      qs('c_name').value = item.name || '';
      qs('c_id').value = item.id || '';
      qs('c_zdp').value = item.zdp || 'WAW';
      qs('c_role').value = item.role || 'KP';
      qs('c_from').value = item.from || '';
      qs('c_to').value = item.to || '';
      formConductor.setAttribute('data-mode','edit');
      formConductor.setAttribute('data-index', idx);
      new bootstrap.Modal(qs('modalConductor')).show();
    } else if (type === 'order') {
      const item = currentReport.sectionD[idx];
      qs('o_number').value = item.number || '';
      qs('o_time').value = item.time || '';
      qs('o_text').value = item.text || '';
      qs('o_source').value = item.source || 'Dyspozytura';
      formOrder.setAttribute('data-mode','edit');
      formOrder.setAttribute('data-index', idx);
      new bootstrap.Modal(qs('modalOrder')).show();
    } else if (type === 'station') {
      const item = currentReport.sectionE[idx];
      qs('s_station').value = item.station || '';
      qs('s_dateArr').value = item.dateArr || '';
      qs('s_dateDep').value = item.dateDep || '';
      qs('s_planArr').value = item.planArr || '';
      qs('s_planDep').value = item.planDep || '';
      qs('s_realArr').value = item.realArr || '';
      qs('s_realDep').value = item.realDep || '';
      qs('s_delayReason').value = item.delayReason || '';
      qs('s_writtenOrders').value = item.writtenOrders || '';
      formStation.setAttribute('data-mode','edit');
      formStation.setAttribute('data-index', idx);
      new bootstrap.Modal(qs('modalStation')).show();
    } else if (type === 'control') {
      const item = currentReport.sectionF[idx];
      qs('f_by').value = item.by || '';
      qs('f_id').value = item.id || '';
      qs('f_desc').value = item.desc || '';
      qs('f_notes').value = item.notes || '';
      formControl.setAttribute('data-mode','edit');
      formControl.setAttribute('data-index', idx);
      new bootstrap.Modal(qs('modalControl')).show();
    } else if (type === 'note') {
      const item = currentReport.sectionG[idx];
      qs('n_text').value = item.text || '';
  ...
  on(previewPdfBtn, 'click', async () => {
    ...
    const makeCrewTable = (title, arr, cols) => {
      ...
      if (arr.length === 0) {
        ...
        arr.forEach(it => {
          const cells = cols.map(k => `<td>${safeText(it[k])}</td>`).join('');
      ...
      <tbody>
        ${currentReport.sectionE.length === 0 ? `<tr><td colspan="11">-</td></tr>` : currentReport.sectionE.map(s => {
          ...
          return `<tr>
            ...
            <td>${safeText(s.dateArr)}</td>
            <td>${safeText(s.planArr)}</td>
            <td>${safeText(s.realArr)}</td>
            <td style="${arrStyle}">${arrVal}</td>
            <td>${safeText(s.dateDep)}</td>
            <td>${safeText(s.planDep)}</td>
            <td>${safeText(s.realDep)}</td>
